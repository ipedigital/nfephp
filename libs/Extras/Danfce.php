...
class Danfce extends CommonNFePHP implements DocumentoNFePHP
{
	...
	public function __construct($docXML = '', $sPathLogo = '', $mododebug = 0, $idToken = '', $emitToken = '', $urlQR = '', $comprovantesTEF = [])
	{
		...
		if ($this->infCpl) {
			$this->infCpl = implode('<br/>',explode(';', $this->infCpl));
		}
	...
	protected function consumidor($dest)
	{
		...
		if (!isset($dest)) {
			...
			if (!empty($consCNPJ)) {
				...
				$consHtml .= "<tr><td colspan=\"3\">CONSUMIDOR <br/> CNPJ: {$consCNPJ} ".
					htmlspecialchars($consNome)."</td></tr>\n";
			} elseif (!empty($consCPF)) {
				$consCPF = $this->pFormat($consCPF, "###.###.###-##");
				$consHtml .= "<tr><td colspan=\"3\">CONSUMIDOR <br/> CPF: {$consCPF} ".
					htmlspecialchars($consNome)."</td></tr>\n";
			} elseif (!empty($considEstrangeiro)) {
				$consHtml .= "<tr><td colspan=\"3\">CONSUMIDOR  <br/> Id. Estrangeiro: {$considEstrangeiro} ".
					htmlspecialchars($consNome)."</td></tr>\n";
			}
			if (!empty($consLgr)&&!empty($consBairro)&&!empty($consMun)&&!empty($consUF)) {
				$consHtml .= "<tr>\n";
				$complemento = empty($consCpl) ? '' : ', ' . $consCpl;
				$consHtml .= "<td colspan=\"3\">".
					htmlspecialchars("{$consLgr}, {$consNro}{$complemento}, {$consBairro}")." - ".
